...
function parseURLVariables(){if(0<window.location.search.length)for(var a=window.location.search.substring(1).split("&"),b=0;b<a.length;b++)if(void 0!==a[b]&&0!==a[b].length&&a[b].includes("=")){var c=a[b].split("="),d=c[0].split("%20").join("_");c=c[1].split("%20").join("").split("%22").join("").split("%27").join("");-1!==def.indexOf(d)?document.getElementById("input-"+d).value=c:-1!==opt.indexOf(d)&&(document.getElementById("input-"+d).value=c,document.getElementById("input-"+d).disabled=!1,document.getElementById("check-"+
    d).checked=!0,document.getElementById("label-"+d).style.color="#107dac")}}function bindEvents(){for(var a=0;a<def.length;a++){var b=document.getElementById("input-"+def[a]);b.onkeypress=0===a?b.onpaste=restrictXYEvent:b.onpaste=restrictSTREvent;b.onblur=blurEvent;b.oninput=inputEvent;validateElement(b)}for(a=0;a<opt.length;a++)b=document.getElementById("input-"+opt[a]),b.onkeypress=b.onpaste=restrictSTREvent,b.onblur=blurEvent,b.oninput=inputEvent,validateElement(b);window.onscroll=scrollable}
