...
function search(){jsonQuery={};for(var a=0;a<def.length;a++){var b=document.getElementById("input-"+def[a]).value.split(" ").join("");""!==b&&(jsonQuery[def[a].split("_").join(" ")]=b)}for(a=0;a<opt.length;a++)b=document.getElementById("input-"+opt[a]).value.split(" ").join(""),""!==b&&(jsonQuery[opt[a].split("_").join(" ")]=b);0===Object.keys(jsonQuery).length?(document.getElementById("results").style.display="none",document.getElementById("warning").style.display="block",$("#warning").animate({opacity:1},
    400,"swing"),document.getElementById("warning").scrollIntoView({behavior:"smooth",block:"end",inline:"nearest"})):(jQuery.extend(jsonQuery,jsonParameters()),html.addClass("waiting"),$.ajax({type:"POST",url:"/cellosaurus-str-search/api/query",data:JSON.stringify(jsonQuery),contentType:"application/json",dataType:"json",success:function(a,b,e){0!==a.results.length?(jsonResponse=a,table.build(a),document.getElementById("results").style.display="table",$("#results").animate({opacity:1},1E3,"swing"),document.getElementById("warning").style.display=
        "none",document.getElementById("caption").scrollIntoView({behavior:"smooth",block:"start",inline:"nearest"})):(document.getElementById("results").style.display="none",document.getElementById("warning").style.display="block",$("#warning").animate({opacity:1},400,"swing"),document.getElementById("warning").scrollIntoView({behavior:"smooth",block:"end",inline:"nearest"}))},error:function(a,b,e){alert("Error: The server is not responding\nPlease contact an administrator")},complete:function(){html.removeClass("waiting")}}))}
...
$(function(){dialogImport=$("#dialog-import").dialog({autoOpen:!1,height:550,width:500,modal:!1,resizable:!1,closeText:null,hide:{effect:"fold",duration:300},show:{effect:"fold",duration:300},buttons:[{text:"Batch Query",disabled:!0,id:"batch",click:function(){html.addClass("waiting");$.ajax({type:"POST",url:"/cellosaurus-str-search/api/batch",data:JSON.stringify(jsonInput),contentType:"application/json",dataType:"text",mimeType:"text/plain; charset=x-user-defined",success:function(a,b,c){b="";for(var d=
                0;d<a.length;d++)b+=String.fromCharCode(a.charCodeAt(d)&255);a=new Uint8Array(b.length);for(d=0;d<b.length;d++)a[d]=b.charCodeAt(d);c=c.getResponseHeader("Content-Type");c=new Blob([a],{type:c});var e=window.URL||window.webkitURL,h=e.createObjectURL(c);c=document.createElement("a");"undefined"===typeof c.download?window.location=h:(c.href=h,c.download="Cellosaurus_STR_Results.zip",document.body.appendChild(c),c.click());setTimeout(function(){e.revokeObjectURL(h)},100);dialogImport.dialog("close")},
